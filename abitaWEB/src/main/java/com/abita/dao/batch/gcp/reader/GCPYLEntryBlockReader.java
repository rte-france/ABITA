...
/**
 * Permet de lire les informations afin d’écrire la pièce comptable YL
 */
public class GCPYLEntryBlockReader implements ItemReader<GCPEntryBlock>, ItemStreamReader<GCPEntryBlock> {
  ...
  /** LOGGER */
  private static final Logger LOGGER = LoggerFactory.getLogger(GCPYLEntryBlockReader.class);
  ...
  public void open(ExecutionContext executionContext) {
    ...
    try {
      List<GCPEntryBlock> gcpEntryBlockList = gcpThirdPartyServiceFacade.getYLEntryBlock(generationDate, DateTimeUtils.MONTHLY_CYCLE);
      if (generationDate.getMonthOfYear() % DateTimeUtils.MONTHS_IN_A_QUARTER == 0) {
        gcpEntryBlockList.addAll(gcpThirdPartyServiceFacade.getYLEntryBlock(generationDate, DateTimeUtils.QUARTERLY_CYCLE));
      }
      if (generationDate.getMonthOfYear() % DateTimeUtils.MONTHS_IN_A_YEAR == 0) {
        gcpEntryBlockList.addAll(gcpThirdPartyServiceFacade.getYLEntryBlock(generationDate, DateTimeUtils.ANNUAL_CYCLE));
      }
      gcpEntryBlockList.addAll(gcpThirdPartyServiceFacade.getYLSporadicallyInvoicingEntryBlock(generationDate));
      gcpEntryBlockList.addAll(gcpThirdPartyServiceFacade.getYLRegularizationBlock(generationDate));
      gcpEntryBlockList.addAll(gcpThirdPartyServiceFacade.getYLRevisionBlock(generationDate));
      gcpEntryBlocks = gcpEntryBlockList.iterator();

    } catch (GcpServiceException e) {
      LOGGER.debug("Erreur lors de la récupération des blocks de lignes pour la pièce comptable YL");
