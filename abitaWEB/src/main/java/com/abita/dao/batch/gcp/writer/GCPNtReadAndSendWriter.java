...
/**
 * Permet d'écrire en base de donnée la somme des montants d'une pièce comptable NT
 * @author
 *
 */
public class GCPNtReadAndSendWriter implements ItemWriter<BigDecimal> {

  /** LOGGER */
  private static final Logger LOGGER = LoggerFactory.getLogger(GCPNtReadAndSendWriter.class);
  ...
  public void write(List<? extends BigDecimal> items) throws HistoryReportingServiceException {
    LOGGER.info("File NT - Writing ...");
    try {
      historyReportingService.create(GcpConstants.TYPE_PIECE_NT, (List<BigDecimal>) items);
